...
const CreateTag: NextPage<{ tag: TagType }> = ({ tag }) => {
  const fileinputref = useRef<any>(null);
  const [title, setTitle] = useState(tag.title);
  const [description, setDescription] = useState(tag.description);
  const [cover, setCover] = useState(tag.cover);
  ...
  async function handleUpdateTag() {
    if (title || description || cover || color || tag.id) {
      let ntag = {
        title,
        description,
        cover,
      ...
      setIsUpdating(true);
      let res = await fetch("/api/tag/update", {
        method: "PATCH",
      ...
      <div className=" p-6 px-10 bg-white flex-1 rounded-lg mt-6">
        <div className=" w-full flex justify-end">
          ...
          <button
            ...
            disabled={isDeleting}
            className=" my-3 mr-3 mt-5 disabled:pointer-events-none disabled:bg-red-300 disabled:border-transparent disabled:text-white text-red-500 border-2 border-red-500 rounded flex items-center p-2 py-1 hover:bg-red-500 hover:text-white hover:border-transparent"
          >
            {isDeleting ? (
              <>
